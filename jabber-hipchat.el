...
(defun hipchat-get-jid ()
  ...
  (car (seq-elt (seq-filter (lambda (acct)
                              (s-match "chat.btf.hipchat.com" (car acct)))
...
(defun hipchat-list ()
  ...
  (interactive)
  (jabber-get-disco-items (hipchat-get-connection) "conf.btf.hipchat.com" nil))
...
(defun hipchat-join ()
  ...
  (jabber-disco-get-items
   (hipchat-get-connection) "conf.btf.hipchat.com" nil
...
(defun hipchat-load-user-info ()
  ...
  (request
   "https://hipchat.monetas.io/v2/user"
