...
Normally I want to install packages only from melpa-stable. However
some packages are only in melpa. Emacs has a "pinning" feature where
you can specify which repository a package should come from.

#+BEGIN_SRC emacs-lisp
  (defun package-install-pinned (repository package)
    "First pins the package to repository, then installs package"
    (add-to-list 'package-pinned-packages (cons package repository))
    (package-install package))

  (defun package-install-stable (package)
    (package-install-pinned "melpa-stable" package))
#+END_SRC

#+RESULTS:
: package-install-stable
...
#+BEGIN_SRC emacs-lisp
  (package-install-stable 'smart-mode-line-powerline-theme)
