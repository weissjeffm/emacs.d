...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Only enable TRAMP direct async processes on Emacs 30+
...
We want to be able to quickly insert code blocks by typing =<s [TAB]=,
need to load org-tempo.

#+begin_src emacs-lisp
(require 'org-tempo)
#+end_src

Install HTMLize so that we can export files
like this one to HTML and get pretty fonts and colors just like we see
in emacs!
...
#+begin_src emacs-lisp
  (use-package org-tanglesync
    ...
    :bind (("C-c M-t" . 'org-tanglesync-process-buffer-automatic)))
#+end_src
